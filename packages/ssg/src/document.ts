 ...
 * child (design 127). The shell's own `<title>` is dropped when the page declares one. The markup
 * goes inside `<body>`, and the body tag gains `data-aweft-ssg`.
 *
 ...
 */
export const buildDocument = (shell: string, parts: DocumentParts): string => {
	...
	const run = `<style ${SHEET}>${parts.css}</style>${parts.head}`;
