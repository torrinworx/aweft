...
// (design 185).
//
// Its own subpath, and every import here is a type or one of the four values `core` and
// `codec` hand out, so a page bundle that reaches for it carries no server module, no store
// and no Node module. Identity is fixed for a connection's life, so this asks once per socket
// and reconnects whenever the cookie changes underneath it.
 ...
 *
 * Throws: `no-origin` when no `origin` was given and there is no page to read one from.
 *
 * Example:
...
export const createAuth = (client: Client, options: AuthOptions = {}): Auth => {
	...
	const route = `${origin}/api/session`;
	...
	return {
		...
		state: <T extends object>(): Handle<T> => {
			...
			//: a second share of a name the peer has already paired waits
			// for an offer that never comes, and so does a share made after `stop()`. So this
			// hands back the same handle for the connection's life. `enter` and `leave` let go
			// of it, and the socket they open offers the topic again.
