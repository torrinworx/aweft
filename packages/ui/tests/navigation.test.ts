...
test('an accordion mints one name and writes it on every section', () => {
	const { render, root, stop } = page(answered(h(Accordion as never, {
		items: [{ label: 'Shipping', content: 'by post' }, { label: 'Returns', content: 'within 30 days' }],
	})));

	assert.equal(root.localName, 'div');
	const sections = allOf(root, 'details');
	assert.equal(sections.length, 2);
	const name = sections[0]?.getAttribute('name') ?? '';
	assert.notEqual(name, '', 'a name was minted off the render');
	assert.equal(sections[1]?.getAttribute('name'), name,
		'and both sections share it, which is what makes the platform keep one open');

	// The line is on the item, so it is the second one that draws it.
	assert.match(rulesOn(render, sections[1]!), /:not\(:first-child\) \{ border-top/);
	assert.equal(root.textContent, 'Shippingby postReturnswithin 30 days');
	stop();
});

test('an accordion given a name uses it, and mounts children as they are', () => {
	const { root, stop } = page(h(Accordion as never, { name: 'facts' },
		h('details', { name: 'facts' }, h('summary', {}, 'One'), 'the first'),
		h('details', { name: 'facts' }, h('summary', {}, 'Two'), 'the second')));

	assert.deepEqual(allOf(root, 'details').map((node) => node.getAttribute('name')),
		['facts', 'facts'], 'the caller\'s own name, on the caller\'s own elements');
	stop();
});

// --- ToggleGroup ---------------------------------------------------------------------------

test('a toggle group is radios for one choice, and the cell holds the item', () => {
	const align = mutable('centre');
	const { root, stop } = page(h(ToggleGroup as never, {
		value: align, options: ['left', 'centre', 'right'], label: 'Alignment',
	}));
	...
	const labels = allOf(root, 'label');
	assert.equal(labels.length, 3, 'one label per option');
	const inputs = allOf(root, 'input');
	assert.deepEqual(inputs.map((node) => node.getAttribute('type')), ['radio', 'radio', 'radio']);
	const name = inputs[0]?.getAttribute('name') ?? '';
	assert.notEqual(name, '');
	assert.deepEqual(inputs.map((node) => node.getAttribute('name')), [name, name, name],
		'one name, which is what makes the arrow keys work with nothing written here');
	assert.deepEqual(inputs.map((node) => node.getAttribute('checked')), [null, '', null]);
	assert.equal(labels[1]?.textContent, 'centre');

	(inputs[2] as unknown as Record<string, unknown>)['checked'] = true;
	fire(inputs[2]!, 'change');
	assert.equal(align.get(), 'right', 'a change wrote the item, not the string the element carries');
	assert.deepEqual(inputs.map((node) => node.getAttribute('checked')), [null, null, '']);
	stop();
});

test('a multiple toggle group is checkboxes, and the cell holds a list in the options\' order', () => {
	const days = mutable<string[]>(['Tue']);
	const seen: unknown[] = [];
	const { root, stop } = page(h(ToggleGroup as never, {
		value: days, options: ['Mon', 'Tue', 'Wed'], multiple: true,
		onChange: (next: unknown) => { seen.push(next); },
	}));

	const inputs = allOf(root, 'input');
	assert.deepEqual(inputs.map((node) => node.getAttribute('type')),
		['checkbox', 'checkbox', 'checkbox']);
	assert.deepEqual(inputs.map((node) => node.getAttribute('checked')), [null, '', null]);

	(inputs[0] as unknown as Record<string, unknown>)['checked'] = true;
	fire(inputs[0]!, 'change');
	assert.deepEqual(days.get(), ['Mon', 'Tue'],
		'the list is rebuilt from the options, so it stays in the order they were declared');

	(inputs[1] as unknown as Record<string, unknown>)['checked'] = false;
	fire(inputs[1]!, 'change');
	assert.deepEqual(days.get(), ['Mon'], 'and clearing one takes it back out');
	assert.deepEqual(seen, [['Mon', 'Tue'], ['Mon']]);
	stop();
});

test('display reads as a function and as a list, exactly as Select reads it', () => {
	const users = [{ id: 1, name: 'Ada' }, { id: 2, name: 'Grace' }];
	const picked = mutable(users[1]);
	const named = page(h(ToggleGroup as never, {
		value: picked, options: users, display: (user: { name: string }) => user.name,
	}));
	assert.deepEqual(allOf(named.root, 'label').map((node) => node.textContent), ['Ada', 'Grace']);
	assert.deepEqual(allOf(named.root, 'input').map((node) => node.getAttribute('checked')), [null, '']);
	named.stop();

	const listed = page(h(ToggleGroup as never, {
		options: ['a', 'b'], display: ['First', 'Second'],
	}));
	assert.deepEqual(allOf(listed.root, 'label').map((node) => node.textContent), ['First', 'Second']);
	listed.stop();
});

test('a toggle group option is a label wearing the button look over a hidden input', () => {
	const { render, root, stop } = page(h(ToggleGroup as never, {
		options: ['one', 'two'], size: 'sm',
	}));

	const item = allOf(root, 'label')[0]!;
	const rules = rulesOn(render, item);
	assert.match(rules, /min-height: 32px/, '$controlSm through the button entry it extends');
	assert.match(rules, /:has\(:checked\) \{/, 'the checked look is the input\'s own state');
	assert.match(rules, /:has\(:focus-visible\) \{[^}]*box-shadow: 0 0 0 3px/,
		'and the ring is on the label, because the input inside it is a pixel nobody can see');

	// Off the screen and still in the focus order, which is what `offscreen` means.
	const input = allOf(root, 'input')[0]!;
	assert.match(rulesOn(render, input), /clip-path: inset\(50%\)/);

	// The corners are joined the way a ButtonGroup joins its buttons.
	assert.match(rulesOn(render, root), /> \* \+ \* \{ margin-left: -1px/);
	stop();
});
