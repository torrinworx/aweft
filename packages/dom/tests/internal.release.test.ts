...
test('in release, a mismatched region is replaced with what the client built', async (t) => {
	...
	// Both sides go through a maker, so the server brackets the same region the client claims.
	const markup = await render(h(() => h('main', {}, h('p', { class: 'x' }, 'hi'), h('b', {}, 'keep'))));
	...
	doc.body.innerHTML = await render(h(() => h('main', {}, 'static')));
	...
	doc.body.innerHTML = await render(h(() => h('main', {}, h('p', {}, 'a'), h('i', {}, 'extra'), mutable('x'))));
	...
	doc.body.innerHTML = await render(h(() => h('p', { class: 'x', id: 'gone' }, 'hi')));
