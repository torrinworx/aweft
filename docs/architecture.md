...
aweft/
  ...
  packages/
    ...
    auth/                  the first battery: the gate, sessions, sign-in, names, per-user state, the mail flows
...
| `auth` | The gate that reads `public` and `needs`, sessions as documents, sign-in and sign-up, the names a person holds (`auth/Roles`: `may`, `grant`, `revoke`, the `implies` table, `first`), the per-user state document, as server modules; a second source, `mail`, for email verification, password change, forgot and reset through `notify/Send`; the client half over a `client` connection: `user` and `names` as cells, `may`, `enter`, `leave`, `state`, `check`, `verify`, `change`, `forgot` and `reset`; and a source of four page modules, `auth/Session`, the `auth/SignIn` form, `auth/Verify` and `auth/Reset` | Which application loads it; which URL any of it is on; which names exist beyond `verified` and what they mean; who grants; who may see a page |
...
| Authority | None in the core. Who may write where is the application's rule, written into a link's `accept` or a node's own code. The auth battery, opt-in, keeps the names a person holds and reads them at its gate and through `may`; which names exist and who grants them stay the application's | 053, 057, 289; 009 superseded |
...
| The gate | Required and outside every module: `identify` once per connection or request answers a context or refuses, `access` runs before a module sees a connection, a call or a request; `open` is the trusted case; a module declares `public`, `needs`, or nothing and the auth gate reads it, `server` does not; the application names a `Gate` object or a module that is one, and a composed gate is a module | 071, 241, 289 |
...
| Who a page is | `auth/client` hands identity as a cell: `undefined`, `null`, or the id, asked over the socket the page opens anyway, and `auth/Session` gains the `call` that answers it. `enter` and `leave` reconnect, because identity is fixed per connection, and `state()` refuses an anonymous connection instead of waiting. `names` is a second cell, following the `roles` share, and `may` runs the server's check over it, for showing and hiding only | 185, 289 |
...
| auth | inside the server recipe: sign up over HTTP, connect with the cookie, the state document shared and persisted, sign out and the old cookie is anonymous; in the backend recipe, the first to sign up holds `admin`, a module declaring `needs` refuses the second until a grant made inside the process reaches them with no reconnect, a per-product name covers one product and not another, a verification link mailed through notify grants `verified`, and a reset link sets the password and ends every session; and, in the client recipe, a page whose every part is a module signs up through `auth/SignIn`, reads `user`, opens `state` and signs out, asks for a reset link on `auth/Reset` and sets the password from the link's page, and takes a verification link on `auth/Verify` with `verified` reaching `names` with no reload, the three forms audited and walked in both modes |
...
by naming it in its acts map. No battery picks a URL. Names are `auth/Roles` (design 289): a
person holds names, dotted and covering what is under them, a table in configuration says
which imply others, the gate reads `needs` off a module against them live, and the page reads
the same document. The mail flows are a second source, `mail` (design 290), that names
`notify/Send` in its `deps`: a verification link grants `verified`, a reset link sets the
password and ends every session, and a change ends every other one.
